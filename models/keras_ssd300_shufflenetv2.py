...
import sys, os
import light_networks.shufflenetv2.shufflenetv2 as shufflenet_v2
import light_networks.shufflenetv2.utils as utils
...
def ssd_300(image_size,
    ...
    shufflenetv2 = shufflenet_v2.ShuffleNetV2(bottleneck_ratio=scale_factor,
                                  ...
                                  include_top=False)
    FeatureExtractor = Model(inputs=shufflenetv2.input, outputs=shufflenetv2.get_layer('stage3/block8/channel_shuffle').output)
    ...
    layer = utils.shuffle_unit(layer, out_channels=out_channels_in_stage[4 - 1], strides=1,
                          bottleneck_ratio=scale_factor, stage=4, block=2)
    layer = utils.shuffle_unit(layer, out_channels=out_channels_in_stage[4 - 1], strides=1,
                          bottleneck_ratio=scale_factor, stage=4, block=3)
    layer = utils.shuffle_unit(layer, out_channels=out_channels_in_stage[4 - 1], strides=1,
                          bottleneck_ratio=scale_factor, stage=4, block=4)
                          
    conv18 = Conv2D(k, kernel_size=1, padding='same', strides=1, name='1x1conv5_out', activation='relu')(layer)
    
    layer, conv19_2 = _conv_blockSSD(conv18, 256, block_id=19)
    layer, conv20_2 = _conv_blockSSD(layer, 128, block_id=20)
    layer, conv21_2 = _conv_blockSSD(layer, 128, block_id=21)
    layer, conv22_2 = _conv_blockSSD(layer, 64, block_id=22)
