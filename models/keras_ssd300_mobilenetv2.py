...
from keras.layers import Input, Lambda, Activation, Conv2D, MaxPooling2D, ZeroPadding2D, Reshape, Concatenate, SeparableConv2D, Dropout, BatchNormalization, Add
...
def ssd_300(image_size,
    ...
    def liteConv(inputs, filters, block_id=11):
        channel_axis = -1
        #filters = int(filters * alpha)
        x = Conv2D(filters, (1, 1), padding='same', use_bias=False, name=str(block_id)+'_pwconv1')(inputs)
        x = BatchNormalization(axis=channel_axis, name=str(block_id)+'_pwconv1_bn')(x)
        x = Activation(relu6, name=str(block_id)+'_pwconv1_act')(x)
        x = DepthwiseConv2D(kernel_size=3, strides=2, activation=None,use_bias=False, padding='same', name=str(block_id)+'_dwconv2')(x)
        x = BatchNormalization(axis=channel_axis, name=str(block_id) + '_sepconv2_bn')(x)
        x = Activation(relu6, name=str(block_id) + '_sepconv2_act')(x)
        net = Conv2D(filters * 2, (1, 1), padding='same', use_bias=False, name=str(block_id) + '_pwconv3')(x)
        x = BatchNormalization(axis=channel_axis, name=str(block_id) + '_pwconv3_bn')(net)
        x = Activation(relu6, name=str(block_id) + '_pwconv3_act')(x)
        return x, net
    ...
    mobilenetv2=MobileNetV2(input_shape=mobilenetv2_input_shape, alpha=alpha, include_top=False,weights=None)
    ...
    layer, conv13 = _isb4conv13(layer, filters=160, alpha=alpha, stride=1,expansion=expansion, block_id=13)
    layer = _inverted_res_block(layer, filters=160, alpha=alpha, stride=1,expansion=expansion, block_id=14)
    layer = _inverted_res_block(layer, filters=160, alpha=alpha, stride=1,expansion=expansion, block_id=15)
    layer = _inverted_res_block(layer, filters=320, alpha=alpha, stride=1,expansion=expansion, block_id=16)    
    layer, conv17 = Conv(layer, 1280)
    ...
    conv13_mbox_conf = Conv2D(n_boxes[0] * n_classes, (3, 3), padding='same', name='conv13_mbox_conf')(conv13)
    conv17_mbox_conf = Conv2D(n_boxes[1] * n_classes, (3, 3), padding='same', name='conv17_mbox_conf')(conv17)
    ...
    conv13_mbox_loc = Conv2D(n_boxes[0] * 4, (3, 3), padding='same', name='conv13_mbox_loc')(conv13)
    conv17_mbox_loc = Conv2D(n_boxes[1] * 4, (3, 3), padding='same', name='conv17_mbox_loc')(conv17)
    ...
    conv13_mbox_conf_reshape = Reshape((-1, n_classes), name='conv11_mbox_conf_reshape')(conv13_mbox_conf)
    conv17_mbox_conf_reshape = Reshape((-1, n_classes), name='conv13_mbox_conf_reshape')(conv17_mbox_conf)
    conv18_2_mbox_conf_reshape = Reshape((-1, n_classes), name='conv14_2_mbox_conf_reshape')(conv18_2_mbox_conf)
    conv19_2_mbox_conf_reshape = Reshape((-1, n_classes), name='conv15_2_mbox_conf_reshape')(conv19_2_mbox_conf)
    conv20_2_mbox_conf_reshape = Reshape((-1, n_classes), name='conv16_2_mbox_conf_reshape')(conv20_2_mbox_conf)
    conv21_2_mbox_conf_reshape = Reshape((-1, n_classes), name='conv17_2_mbox_conf_reshape')(conv21_2_mbox_conf)
    
    # Reshape the box predictions, yielding 3D tensors of shape `(batch, height * width * n_boxes, 4)`
    # We want the four box coordinates isolated in the last axis to compute the smooth L1 loss
    conv13_mbox_loc_reshape = Reshape((-1, 4), name='conv11_mbox_loc_reshape')(conv13_mbox_loc)
    conv17_mbox_loc_reshape = Reshape((-1, 4), name='conv13_mbox_loc_reshape')(conv17_mbox_loc)
    conv18_2_mbox_loc_reshape = Reshape((-1, 4), name='conv14_2_mbox_loc_reshape')(conv18_2_mbox_loc)
    conv19_2_mbox_loc_reshape = Reshape((-1, 4), name='conv15_2_mbox_loc_reshape')(conv19_2_mbox_loc)
    conv20_2_mbox_loc_reshape = Reshape((-1, 4), name='conv16_2_mbox_loc_reshape')(conv20_2_mbox_loc)
    conv21_2_mbox_loc_reshape = Reshape((-1, 4), name='conv17_2_mbox_loc_reshape')(conv21_2_mbox_loc)
